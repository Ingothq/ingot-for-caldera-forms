    ...
    grunt.initConfig({
        ...
        gitcommit: {
            commit: {
                ...
                files: {
                    src: [ 'package.json', 'readme.txt', 'cf-stripe.php', 'releases/<%= pkg.name %>-<%= pkg.version %>.zip' ]
                }
            }
        },
        gitpush: {
        ...
        replace: {
            core_file: {
                src: [ 'ingot-for-caldera-forms.php' ],
                overwrite: true,
                replacements: [{
                    from: /Version:\s*(.*)/,
                    to: "Version: <%= pkg.version %>"
                }, {
                    from: /define\(\s*'INGOT_CF_VER',\s*'(.*)'\s*\);/,
                    to: "define( 'INGOT_CF_VER', '<%= pkg.version %>' );"
                }]
            }
        }
